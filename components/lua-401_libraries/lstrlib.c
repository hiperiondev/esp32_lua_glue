...
static const char* matchbalance(lua_State *L, const char *s, const char *p, struct Capture *cap) {
    if (*p == 0 || *(p + 1) == 0)
...
static const char* max_expand(lua_State *L, const char *s, const char *p, const char *ep, struct Capture *cap) {
...
static const char* min_expand(lua_State *L, const char *s, const char *p, const char *ep, struct Capture *cap) {
    for (;;) {
...
static const char* start_capture(lua_State *L, const char *s, const char *p, struct Capture *cap) {
    const char *res;
    int level = cap->level;
...
static const char* end_capture(lua_State *L, const char *s, const char *p, struct Capture *cap) {
...
static const char* match_capture(lua_State *L, const char *s, int level, struct Capture *cap) {
            ...
            { /* it is a pattern item */
                ...
                switch (*ep) {
                    ...
                    default:
                        ...
                        p = ep;
                        goto init;
                        /* else return match(L, s+1, ep, cap); */
                }
            }
    }
}
...
static int str_gsub(lua_State *L) {
    ...
    struct Capture cap;
    luaL_Buffer b;
    luaL_arg_check(L, lua_gettop(L) >= 3 && (lua_isstring(L, 3) || lua_isfunction(L, 3)), 3, "string or function expected");
    luaL_buffinit(L, &b);
