...
static int number_constant(FuncState *fs, Number r) {
    ...
    luaM_growvector(fs->L, f->knum, f->nknum, 1, Number, "constant table overflow", MAXARG_U);
    c = f->nknum++;
    f->knum[c] = r;
    return c;
}
...
static void luaK_patchlistaux(FuncState *fs, int list, int target, OpCode special, int special_target) {
...
void luaK_tostack(LexState *ls, expdesc *v, int onlyone) {
    ...
    if (!discharge(fs, v)) { /* `v' is an expression? */
        ...
        if (!ISJUMP(previous) && v->u.l.f == NO_JUMP && v->u.l.t == NO_JUMP) {
            ...
            if (ISJUMP(previous) || need_value(fs, v->u.l.f, OP_JMPONF) || need_value(fs, v->u.l.t, OP_JMPONT)) {
...
static void codelineinfo(FuncState *fs) {
    ...
    if (ls->lastline > fs->lastline) {
        luaM_growvector(fs->L, f->lineinfo, f->nlineinfo, 2, int, "line info overflow", MAX_INT);
...
int luaK_code2(FuncState *fs, OpCode o, int arg1, int arg2) {
    ...
    luaM_growvector(fs->L, fs->f->code, fs->pc, 1, Instruction, "code size overflow", MAX_INT);
    fs->f->code[fs->pc] = i;
    return fs->pc++;
}
#endif
