...
void luaS_freeall(lua_State *L) {
    ...
    luaM_free(L, L->strt.hash); LUA_ASSERT(L->udt.nuse==0, "non-empty udata table");
...
TString* luaS_newudata(lua_State *L, size_t s, void *udata) {
    union L_UTString *uts = (union L_UTString*) luaM_malloc(L, (lint32 )sizeof(union L_UTString) + s);
    TString *ts = &uts->ts;
    ts->marked = 0;
    ts->nexthash = NULL;
    ts->len = s;
