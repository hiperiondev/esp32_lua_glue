...
static void check_match(LexState *ls, int what, int who, int where) {
    if (ls->t.token != what) {
        if (where == ls->linenumber)
            ...
            sprintf(buff, "`%.20s' expected (to close `%.20s' at line %d)", t_what, t_who, where);
            luaK_error(ls, buff);
        }
    }
    next(ls);
...
static int string_constant(FuncState *fs, TString *s) {
    ...
    if (c >= f->nkstr || f->kstr[c] != s) {
        luaM_growvector(fs->L, f->kstr, f->nkstr, 1, TString*, "constant table overflow", MAXARG_U);
        c = f->nkstr++;
        f->kstr[c] = s;
...
static void pushupvalue(LexState *ls, TString *n) {
    ...
    if (level == -1) { /* global? */
        ...
        luaX_syntaxerror(ls, "upvalue must be global or local to immediately outer scope", n->str);
...
static void pushclosure(LexState *ls, FuncState *func) {
    ...
    luaM_growvector(ls->L, f->kproto, f->nkproto, 1, Proto*, "constant table overflow", MAXARG_A);
...
Proto* luaY_parser(lua_State *L, ZIO *z) {
    ...
    close_func(&lexstate);
    LUA_ASSERT(funcstate.prev == NULL, "wrong list end"); LUA_ASSERT(funcstate.nupvalues == 0, "no upvalues in main");
    return funcstate.f;
}
...
static int listfields(LexState *ls) {
    ...
    while (ls->t.token == ',') {
        ...
        exp1(ls);
        n++;
        luaX_checklimit(ls, n / LFIELDS_PER_FLUSH, MAXARG_A, "`item groups' in a list initializer");
...
static void forlist(LexState *ls, TString *indexname) {
    ...
    check_condition(ls, (ls->t.token == TK_NAME && ls->t.seminfo.ts == luaS_new(ls->L, "in")), "`in' expected");
    next(ls); /* skip `in' */
    exp1(ls); /* table */
