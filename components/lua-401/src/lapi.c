...
LUA_API int lua_ref(lua_State *L, int lock) {
    ...
    if (ttype(L->top-1) == LUA_TNIL)
        ...
        if (L->refFree != NONEXT) { /* is there a free place? */
            ...
            luaM_growvector(L, L->refArray, L->refSize, 1, struct Ref, "reference table overflow", MAX_INT);
...
LUA_API void lua_settag(lua_State *L, int tag) {
    ...
    switch (ttype(L->top - 1)) {
        ...
        case LUA_TUSERDATA:
            ...
            break;
        default:
            luaO_verror(L, "cannot change the tag of a %.20s", luaO_typename(L->top - 1));
    }
}
