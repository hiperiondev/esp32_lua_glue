...
static int LoadHeader(lua_State *L, ZIO *Z) {
    ...
    if (version > VERSION)
        luaO_verror(L, "`%.99s' too new:\n"
                "  read version %d.%d; expected at most %d.%d", ZNAME(Z), V(version), V(VERSION));
    if (version < VERSION0) /* check last major change */
        luaO_verror(L, "`%.99s' too old:\n"
                "  read version %d.%d; expected at least %d.%d", ZNAME(Z), V(version), V(VERSION));
...
Proto* luaU_undump(lua_State *L, ZIO *Z) {
    ...
    if (c != EOZ)
        ...
        printf("WARNING: `%.99s' apparently contains more than one chunk\n\n", ZNAME(Z));
    return tf;
}
